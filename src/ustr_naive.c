...
static inline int32_t naive__bytesize(const char *s) {
    return *(const int32_t *)(s - sizeof(int32_t));
}

static inline const uint8_t *naive__marks(const char *s, int32_t size) {
...
char *ustr_slice_naive(const char *s, int32_t start, int32_t end) {
    ...
    for (int32_t i = 0; i <= size && cp <= end; i++) {
        ...
        if (is_mark) {
            if (cp == start) byte_start = i;
            if (cp == end)   byte_end = i;
            cp++;
        }
    }
    if (byte_start < 0) return ustr_new(0, "");
    if (byte_end < 0)   byte_end = size;
