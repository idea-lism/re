...
static void run_validate_bench(const char *label, const char *data, size_t sz) {
    ...
    printf("%-20s  %8zu bytes  scalar: %6.2f ns/byte  neon: %6.2f ns/byte  speedup: %.2fx\n",
           label, sz, scalar_ns, neon_ns, speedup);
}
...
static void bench_cplen(const char *label, size_t sz, char *(*make)(size_t)) {
    char *s = make(sz);
    if (!s) { printf("%-20s  SKIP (ustr_new failed)\n", label); return; }

    int iters = 10000;
    volatile int32_t sink;

    for (int i = 0; i < 100; i++) sink = ustr_size(s);
    for (int i = 0; i < 100; i++) sink = ustr_size_naive(s);

    double t0 = now_sec();
    for (int i = 0; i < iters; i++) sink = ustr_size(s);
    double t_popcnt = now_sec() - t0;

    t0 = now_sec();
    for (int i = 0; i < iters; i++) sink = ustr_size_naive(s);
    double t_naive = now_sec() - t0;

    (void)sink;
    double speedup = t_naive / t_popcnt;
    printf("%-20s  %8zu bytes  naive: %8.1f ns  popcnt: %8.1f ns  speedup: %.2fx\n",
           label, sz, t_naive / iters * 1e9, t_popcnt / iters * 1e9, speedup);
...
static void bench_slice(const char *label, size_t sz, char *(*make)(size_t)) {
    ...
    for (int i = 0; i < 50; i++) { char *t = ustr_slice(s, quarter, mid); ustr_del(t); }
    for (int i = 0; i < 50; i++) { char *t = ustr_slice_naive(s, quarter, mid); ustr_del(t); }
